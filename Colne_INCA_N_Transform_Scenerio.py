...
def Colne_INCA_N_transforamtion():
    try:
        ...
        sb_c = df.loc[df['Flow'].str.contains("Reach ", case=False)]
        sb_c['Flow'] = sb_c['Flow'].str.replace('Reach ', '')
        ...
        index_del = []
        for i in sb_c['Flow']:
            ind = sb_c.index[sb_c['Flow'] == i].tolist()
            r1 = ind[0]+1
            r2 = ind[0]+2
            r3 = ind[0]+3
            ...
            elif p > 0 and p < (len(sb_c['Flow'])+1):
        ...
        temp_csv_file = 'Colne_INCA-N_temp.csv'
